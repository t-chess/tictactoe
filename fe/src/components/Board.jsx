            ...
            <div className='p-2'>
                ...
                <div className='flex flex-col gap-8'>
                    <div className='flex gap-4 [&_input]:border [&_input]:w-16'>
                        <Input name="rows" label="Rows" value={rows} max={10} onChange={(v)=>{setRows(Number(v));resetHistory()}} />
                        <Input name="cols" label="Cols" value={cols} max={10} onChange={(v)=>{setCols(Number(v));resetHistory()}} />
                        <Input name="numToWin" label="To win" value={numToWin} max={Math.min(rows,cols)} onChange={(v)=>{setNumToWin(Number(v));resetHistory()}} />
                    </div>
