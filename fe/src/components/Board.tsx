...
const Board = ({ id, onDelete = () => {}, onCreate = () => {} }: BoardProps) => {
  ...
  const handleClick = (i: number) => {
    ...
    setStep(nextStep);
    setHistory((prev) => [
      ...prev.slice(0, nextStep),
      { board: nextState, winner: check(i, nextState, who) ? who : undefined },
    ]);
  };
  const check = (index: number, state: BoardData, who: string) => {
    ...
    let diag1 = '';
    while (dr1 < rows && dc1 < cols) {
      diag1 += state[dr1 * cols + dc1] ?? '_';
      dr1++;
      dc1++;
    }
    ...
    // diag lb to rt
    let dr2 = Math.floor(index / cols),
      dc2 = cs;
    while (dr2 < rows - 1 && dc2 > 0) {
      dr2++;
      dc2--;
    }
    let diag2 = '';
    while (dr2 >= 0 && dc2 < cols) {
      diag2 += state[dr2 * cols + dc2] ?? '_';
      dr2--;
      dc2++;
    }
    ...
    <div className="p-4">
      ...
      <div className="p-2">
        <div
          ...
          style={{ gridTemplateRows: `repeat(${rows},100px)`, gridTemplateColumns: `repeat(${cols},100px)` }}
        >
          {cells.map((i) => (
