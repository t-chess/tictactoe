    ...
    <div className='flex justify-between gap-8 p-2'>
      ...
      <div className='flex flex-col gap-8'>
        <div className='flex gap-4 [&_input]:border [&_input]:w-16'>
          <label><p>Rows</p><input type="number" value={rows} max={10} onChange={(e)=>setRows(Number(e.target.value))} /></label>
          <label><p>Cols</p><input type="number" value={cols} max={10} onChange={(e)=>setCols(Number(e.target.value))} /></label>
          <label><p>To win</p><input type="number" value={numToWin} max={Math.min(rows,cols)} onChange={(e)=>setNumToWin(Number(e.target.value))} /></label>
        </div>
        <strong className='border-t border-b'>{winner?`Winner: ${winner}`:`Now: ${xIsNext?"X":"O"}`}</strong>
        <div className='flex flex-col items-start'>
          {history.map((_,i)=><button className='hover:bg-slate-100 cursor-pointer' key={i} onClick={()=>setStep(i)}>{i===0?"Go to start":`Go to move #${i}`}</button>)}
        </div>
      </div>
    </div>
  )
