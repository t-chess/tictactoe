...
export const isBoard = (b: unknown): b is (string | null)[] => Array.isArray(b) && b.every(v => v === null || typeof v === "string");

export const isHistoryItem = (h: unknown): h is HistoryData => !!h && typeof h === "object" && isBoard((h as any).board) &&
...
export const validateNewBody = (b: any): { ok: true; data: NewGameBody } | { ok: false; error: string } => {
    ...
    if (!Number.isInteger(numToWin) || numToWin < 1 || numToWin > Math.max(rows, cols)) return { ok: false, error: "numToWin must be an integer" };
